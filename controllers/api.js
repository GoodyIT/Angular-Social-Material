...
var SCHEDULER_SERVER = process.env.SCHEDULER_SERVER || "http://la8bel-agent-dev.elasticbeanstalk.com";
...
exports.getLambdaUrl = function(req, res){
    res.send(secrets.lambda.endPoint);
};
/**
 * GET /api/foursquare
...
exports.schedulePostsForApplication = function(req, res, nect){
    ...
    var schedulePost = function(platform, pageId, actionTime, accessToken, postContent, userId, type, imgUrl, shortLink, mobileLink, ownerId, callback){
        var sendPost = function(accessToken){
            if(accessToken){
                // TODO: extend token if needed
                console.log(mobileLink);
                if(mobileLink){
                    if(mobileLink.iosTracking){
                        postContent += "\n For iOS Downloads:" + mobileLink.iosTracking;
                    }
                    if(mobileLink.androidTracking){
                        postContent += "\n For Android Downloads:" + mobileLink.androidTracking;
                    }
                }
                ...
                var scheduleOptions = {
                    method: 'POST',
                    url: secrets.lambda.endPoint + '/schedulepost',
                    body:  {
                        applicationId : trade.applicationId,
                        campaignId: trade.campaignId,
                        actionTime: actionTime,
                        message: postContent,
                        campaignOwnerUserId: ownerId,
                        type: type,
                        social: platform,
                        link: shortLink  || "",
                        linkCaption: postContent,
                        linkType: "OPEN_LINK"
                    },
                    headers: {
                ...
                if(platform === 'facebook'){
                    scheduleOptions.body.pageId = pageId;
                    scheduleOptions.body.accessToken = accessToken;
                } else if(platform === 'twitter'){
                    scheduleOptions.body.accessToken = accessToken.accessToken;
                    scheduleOptions.body.accessSecret = accessToken.tokenSecret;
                    console.log("accessToken:" + accessToken.accessToken);
                    console.log("accessToken:" + accessToken.tokenSecret);
                }
                scheduleOptions.body = JSON.stringify(scheduleOptions.body);
                console.log("scheduleOptions.body: " + scheduleOptions.body);
        ...
        if(!accessToken){
            userController.getUser(userId, null, function(err, user) {
                if(user){
                    if(user.tokens){
                        var fbToken;
                        var twitterToken;
                        for (var i=0; i< user.tokens.length; i++){
                            if(platform === 'facebook' && user.tokens[i].kind == 'facebook'){
                                fbToken = user.tokens[i].accessToken;
                                sendPost(fbToken);
                                break;
                            }
                            if(platform === 'twitter' && user.tokens[i].kind == 'twitter'){
                                twitterToken = user.tokens[i];
                                sendPost(twitterToken);
                                break;
                            }
                        }
                    }else {
                        res.status(500).send("Cannot get user token.");
                    }
                }
            });
        }else{
    ...
    if(trade.actionTime){
        //For trade campagin
        schedulePost(trade.platform, trade.facebookPageId, trade.actionTime, trade.applicantAccessToken, trade.postContent, trade.userId, "applicant", trade.imgUrl, trade.campaignShortLink,
            {androidTracking:trade.androidTracking, iosTracking:trade.iosTracking}, trade.ownerUserId, function(response){
            if(trade.ownerActionTime){
                schedulePost(trade.platform, trade.ownerFacebookPageId, trade.ownerActionTime, null, trade.ownerPostContent, trade.ownerUserId, "owner", trade.ownerImgUrl, null, null, trade.ownerUserId, function(response){return res.send(response)});
            }else {
        ...
        console.log(trade);
        console.log(trade.socialId + " time: " + trade.actionTime);
